...
# elast_path = 'uniform_elasticities_4.csv'
...
list_of_elasticities = [
#     "sigmaNewConcordance.csv",
# "sigmaNewConcordanceNoAgNoEl.csv",
# "sigmaOldConcordance.csv",
# "rescaled_to_4elasticities_agg1.csv",
# "rescaled_to_5elasticities_agg1.csv",
# "elasticities_agg1.csv",
# "cp_estimate_allyears.csv"
"uniform_elasticities_4.csv"
]

for elast_path in list_of_elasticities:

    # save_path = 'presentation_material/elast_compar_'+elast_path[:-4]+'_no_direct_emissions/'
    save_path = 'presentation_material/elast_compar_'+elast_path[:-4]+'/'
    ...
    try:
        ...
        pass
    # try:
    #     os.mkdir(save_path+'sectors/')
    # except:
    #     pass
    # try:
    #     os.mkdir(save_path+'tilt/')
    # except:
    #     pass
    ...
    sols, baselines, relevant_runs, found_cases, not_found_cases = t.sol.load_sols(cases,
                                                          ...
                                                          keep='last',
                                                          exclude_direct_emissions=exclude_direct_emissions
                                                          )
    ...
    def epsilon_i(trade,e):
        return np.einsum('is,is,,->i',
                         np.einsum('isj->is',trade),
                         e,
                         1/np.einsum('isj->',trade),
                         1/e_scal(trade,e))
    ...
    def alpha_i(trade):
        num = np.einsum('isj->i',trade)
        denom = np.einsum('isj->',trade)
        return np.einsum('i,->i',num,1/denom)
    
    def rca_is(trade):
        return np.einsum('is,i->is',
                         alpha_is(trade),
                         1/alpha_i(trade)
                         )
    l_term_1 = []
    ...
    for i in tqdm(range(len(sols)-1)):
        ...
        term_3 = np.einsum('i,i,i->i',
                   epsilon_i(trade_baseline,e),
                   alpha_i(trade_cf)-alpha_i(trade_baseline),
                   1/alpha_i(trade_baseline))
    
        term_4 = np.einsum('is,is,is->is',
                   epsilon_is(trade_baseline,e),
                   rca_is(trade_cf)-rca_is(trade_baseline),
                   1/rca_is(trade_baseline))
    ...
    print('here')
    pio.write_image(fig3,save_path+'term_2_sunburst.pdf', format='pdf', 
                    # engine='orca',
    ...
    pio.write_image(fig3,save_path+'term_3_sunburst.pdf', format='pdf', 
                    # engine='orca',
                    width=4000*scaling,height=4000*scaling*21/29.7)
    ...
    #%% plot term 3 epsilon(alpha) i specific for each sector
    
    # trade_baseline = trade['baseline'].values.reshape((N,S,N))
    # trade_cf = trade[10].values.reshape((N,S,N)) 
    
    # for s,sector in enumerate(b.sector_list):
    ...
    #     data_base['y'] = epsilon_is_tilde(trade_baseline,e)[:,s]
    #     data_base['x'] = alpha_is(trade_baseline)[:,s]
    #     # for g,group_label in enumerate(data_base.group_label.drop_duplicates()):
    #     data = data_base.loc[(data_base['x'] > 0) & (data_base['y'] > 0)]
    #     # data= data_base
    #     ax.scatter(data.x,data.y
    #                # , label = group_label,color=colors[g]
    #                )
    ...
    #     ax.set_xlabel(r'$\alpha_{is}$',fontsize = 30)
    #     ax.set_ylabel(r'$\tilde{\epsilon}_{is}$',fontsize = 30,rotation=0,labelpad=30)
    #     ax.plot([0,data_base.x.max()],[0,data_base.x.max()],ls='--',color='k')
    #     plt.title('Epsilon of alpha for sector '+sector_map.loc[sector,'industry'])
    ...
    #                             ))
    #     if save or save_all:
    #         plt.savefig(save_path+'sectors/term_3_eps_alpha_i_'+sector_map.loc[sector,'industry']+'.'+save_format,format=save_format)
    ...
    # #%% plot term 3 dalpha/ alpha function of epsilon full
    ...
    # data_base['y'] = (alpha_is(trade_cf)-alpha_is(trade_baseline)).ravel()/alpha_is(trade_baseline).ravel()
    # data_base['x'] = epsilon_is(trade_baseline,e).ravel()
    # data = data_base
    # ax.scatter(data.x,data.y)
    # texts = [plt.text(data.x.iloc[i], 
    #         data.y.iloc[i], 
    ...
    data['rca_change'] = np.einsum('is->is',
               rca_is(trade_cf)-rca_is(trade_baseline)
               ).ravel()
    # data['rca_change'] = np.einsum('is,is->is',
    #            rca_is(trade_cf)-rca_is(trade_baseline),
    #            1/rca_is(trade_baseline)).ravel()
    ...
    ax.scatter(data['epsilon'],data['rca_change'])
    ax.set_xlabel(r'$\epsilon_{is}$')
    # ax.scatter(b.co2_intensity.value,data['rca_change'])
    # ax.set_xlabel('CO2 intensity')

    ax.set_xscale('log')
    
    
    ax.set_ylabel('RCA change')
