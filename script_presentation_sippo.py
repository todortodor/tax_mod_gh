...
carb_taxes = np.array([sol.params.carb_cost*1e6  for sol in sols])
# norm_emissions = np.array([(sol.output.value.values*b.co2_intensity.value.values/1e6).sum()
norm_emissions = np.array([sol.co2_prod.value.sum()
                           # /(baselines[y].output.value.values*b.co2_intensity.value.values/1e6).sum()
...
texts = []
data_base = sector_map.copy()
# data_base['group_code'] = data_base['group_code'].replace(6,5)
# data_base['group_code'] = data_base['group_code'].replace('Logistics','Services')
...
# for g,group_label in enumerate(data_base.group_label_v2.drop_duplicates()):
#     data = data_base.loc[data_base['group_label_v2'] == group_label]
#     ax.scatter(data.x,data.y, label = group_label,
#                 color=colors[g],#s=250#,marker = markers[g]
#                )
    # texts_group = [plt.text(data.x.iloc[i], 
    #         data.y.iloc[i], 
    #         industry,
...
ax.set_ylabel('Growth rate (%)',
              fontsize = 20,rotation=90)#,labelpad=30)
# ax.plot([0,data_base.x.max()],[0,data_base.x.max()],ls='--',color='k')
# ax.set_xlim([1e-2,3e1])
...
for i,sector in enumerate(b.sector_list):
    data_base.loc[('Expenditure decrease',sector_map.loc['D'+sector,'industry'],''),'value'] = term2[i]*100
...
for i,sector in enumerate(b.sector_list):
    data_base.loc[('Emissions decrease',sector_map.loc['D'+sector,'industry'],''),'value'] = term2[i]*100
...
#%% Tilt of reorganisation for all sectors

save_path_tilt = save_path+'tilt/'

try:
    os.mkdir(save_path_tilt)
except:
    pass
...
for sector in sector_list:
    ...
    data_base['x'] = alpha_is(trade_baseline)[:,sector_list.index(sector)]\
        /epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]
    data_base['y'] = alpha_is(trade_cf)[:,sector_list.index(sector)]-alpha_is(trade_baseline)[:,sector_list.index(sector)]
    
    ax.scatter(data_base.x,data_base.y)
    
    texts = [plt.text(data_base.x.iloc[i], 
            data_base.y.iloc[i], 
            country,
            size=15,
            # color=colors[g]
            ) 
            for i,country in enumerate(data_base.country_name)]
    
    fit = np.polyfit(data_base.x.values[epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]!=0],
                     data_base.y.values[epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]!=0],
                     1)
    
    ax.plot(data_base.x.values[epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]!=0],
            np.polyval(fit,data_base.x.values[epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]!=0]),
            color='red')
    
    ax.set_ylabel('d(alpha)')
    ax.set_xlabel('alpha/epsilon (Green comparative advantage)')
    ...
    plt.title(sector_map.loc[sector,'industry'])
    # plt.savefig(save_path_tilt+sector_map.loc[sector,'industry']+'.png',format='png')
    plt.show()

#%% Collecting all the fits
...
for sector in sector_list:
    print(sector)
    x = (alpha_is(trade_baseline)[:,sector_list.index(sector)]\
        /epsilon_is(trade_baseline,e)[:,sector_list.index(sector)])[epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]!=0]
    y = (alpha_is(trade_cf)[:,sector_list.index(sector)]\
         -alpha_is(trade_baseline)[:,sector_list.index(sector)])[epsilon_is(trade_baseline,e)[:,sector_list.index(sector)]!=0]
    fit[sector] = np.polyfit(x,y,1)
    # if x.max()>xmax:
    #     xmax = x.max()
    # if x.min()<xmin:
    #     xmin = x.min()

    ax.plot(sorted(x),np.polyval(fit[sector],sorted(x)),label=sector_map.loc[sector,'industry'])

# for sector in sector_list:
#     ax.plot([xmin,xmax],np.polyval(fit[sector],[xmin,xmax]))
# plt.legend(loc='center right')
plt.legend()
plt.xscale('log')
# plt.savefig(save_path+'tilt_superimposed_log.png',format='png')

plt.show()

#%% Plotting the slopes as a function of epsilon_s

fig, ax = plt.subplots(figsize=(25,15))

x = epsilon_s(trade_baseline,e)
y = [fit[sector][0] for sector in sector_list]
...
texts = [plt.text(x[i], 
        ...
        for i,sector in enumerate(sector_map.industry)]

ax.set_xlabel('Epsilon_s of the sector')
ax.set_ylabel('Tilt of the reallocation curve')

plt.xscale('log')
plt.yscale('symlog', linthresh=1e-9)
# plt.savefig(save_path+'tilt_correlated_to_average_carbon_intensity.png',format='png')

plt.show()

#%% share traded change function of share traded

def alpha_s_domestic(trade):
    num = np.einsum('isi->s',trade)
    denom = np.einsum('isj->s',trade)
    return num/denom
...
texts = []
data_base = sector_map.copy()
data_base['y'] = (alpha_s_domestic(trade_cf) 
                  - alpha_s_domestic(trade_baseline))*100
data_base['x'] = alpha_s_domestic(trade_baseline)*100
...
optimizedParameters, pcov = opt.curve_fit(func, data_base.x.values, data_base.y.values)

# Use the optimized parameters to plot the best fit
ax.plot(np.sort(data_base.x.values), 
        func(np.sort(data_base.x.values), *optimizedParameters), 
        label="fit",ls='--',color='grey')
ax.set_xlabel('Share of goods produced domestically (%)',fontsize = 30)
ax.set_ylabel('Change in share of goods produced domestically (pp)',fontsize = 30)
# ax.set_xlim([1e-2,3e1])
plt.legend(fontsize = 25)
# plt.xscale('log')
# plt.title('Term 2 intermediate input only', fontsize = 30)
...
# plt.savefig(save_path+'change_in_alpha_domestic_function_of_carbon_intensity.png',format='png')

plt.show()

# fig, ax = plt.subplots(figsize=(25,15))

#%% share traded change function of emissions intensity of sector
...
data_base = sector_map.copy()
data_base['y'] = (alpha_s_domestic(trade_cf) 
                  - alpha_s_domestic(trade_baseline))/alpha_s_domestic(trade_baseline)
data_base['x'] = e_s(trade_baseline,e)
...
ax.plot(np.sort(data_base.x.values), 
        func(np.sort(data_base.x.values), *optimizedParameters), 
        label="fit",ls='--',color='grey')
ax.set_xlabel('Emissions intensity of the sector',fontsize = 30)
ax.set_ylabel('Change in share of goods produced domestically (pp)',fontsize = 30)
# ax.set_xlim([1e-2,3e1])
plt.legend(fontsize = 25)
plt.xscale('log')
...
#%% share traded change function of standard deviations of emissions intensity of sector
...
data_base['y'] = (alpha_s_domestic(trade_cf) 
                  - alpha_s_domestic(trade_baseline))/alpha_s_domestic(trade_baseline)
data_base['x'] = np.std(e,axis=0)/e_s(trade_baseline,e)
# data_base['x'] = np.std(e,axis=0,where=e>np.sort(e,axis=1)[-10,:])/e_s(trade_baseline,e)
...
ax.plot(np.sort(data_base.x.values), 
        func(np.sort(data_base.x.values), *optimizedParameters), 
        label="fit",ls='--',color='grey')
# fit = np.polyval(np.polyfit(data_base.x.values,
#                  data_base.y.values,
#                  1),data_base.x.values)
# ax.plot(data_base.x,fit,ls='--',color='grey')
ax.set_xlabel('Normalized standard deviation of emssions intensities across countries',fontsize = 30)
ax.set_ylabel('Change in share of goods produced domestically (pp)',fontsize = 30)
# ax.set_xlim([1e-2,3e1])
plt.legend(fontsize = 25)
# plt.xscale('log')
...
#%% share traded change function of normalized standard deviations of emissions intensity of sector

df = b.co2_intensity.reset_index().copy()
df = df.pivot(index='sector',columns='country',values='value')

df_mask = b.output.reset_index().copy()
df_mask = df_mask.pivot(index='sector',columns='country',values='value')

# std = np.std(df.values,axis=1,where=(df_mask.values>df_mask.median(axis=1).values[:,None]))
# mean = np.mean(df.values,axis=1,where=(df_mask.values>df_mask.median(axis=1).values[:,None]))
std = np.std(df.values,axis=1)
mean = np.mean(df.values,axis=1)

data_base = sector_map.copy()
data_base['y'] = (alpha_s_domestic(trade_cf) 
                  - alpha_s_domestic(trade_baseline))
data_base['x'] = std/mean
# data_base['x'] = np.std(e,axis=0,where=e>np.sort(e,axis=1)[-10,:])/e_s(trade_baseline,e)
...
#%% share traded change function of share of transport input

df = b.iot.reset_index().copy()
df = df.loc[df.row_sector.isin(['49','50','51'])]
df = df.groupby('col_sector')['value'].sum()
df = df/b.iot.reset_index().groupby('col_sector')['value'].sum()
...
#                  1),data_base.x.values)
# ax.plot(data_base.x,fit,ls='--',color='grey')
ax.set_xlabel('Share of transport as intermediate input',fontsize = 30)
ax.set_ylabel('Change in share of goods produced domestically (pp)',fontsize = 30)
# ax.set_xlim([1e-2,3e1])
plt.legend(fontsize = 25)
plt.xscale('log')
...
data = pd.DataFrame(index = pd.MultiIndex.from_product([country_list,sector_map.index.get_level_values(level=0).to_list()],
                                                       names=['country','sector']))
                    # columns=['value']
fig,ax =plt.subplots(figsize=(12,8),constrained_layout=True)
# data = data.reset_index().merge(sector_map.reset_index(),how='left',left_on='sector',right_on='ind_code'
#                                 ).set_index(['country','sector']).drop('ind_code',axis=1)
data['term_3'] = np.einsum('is,is->is',
            alpha_is(trade_cf),#-alpha_is(trade_baseline),
            1/alpha_is(trade_baseline)).ravel()
# data['epsilon'] = np.einsum('is->is',
#             epsilon_is(trade_baseline,e)).ravel()
# ax.scatter(data['epsilon'],data['term_3'])
ax.set_xlabel('Emissions intensity (tCO2eq/Mio.$)',fontsize=20)
ax.set_ylabel('Growth rate (%)',fontsize=20)
# from scipy.interpolate import splrep, BSpline
# x_spl = np.log(data.sort_values('epsilon')['epsilon'].values)
# y_spl = data.sort_values('epsilon')['term_3'].values
# fit = np.polyfit(x_spl[x_spl>-np.inf], y_spl[x_spl>-np.inf], 10)
# p = np.poly1d(fit)
# ax.plot(np.exp(x_spl[x_spl>-np.inf]),p(np.exp(x_spl[x_spl>-np.inf])))
# tck = splrep(data['epsilon'].sort_values(), data['term_3'], s=40)
# ax.plot(x_spl, BSpline(*tck)(x_spl),color='red')

# data['term_3'] = np.einsum('is->is',
#             alpha_is(trade_cf)-alpha_is(trade_baseline)).ravel()
# data['epsilon'] = np.einsum('is,is->is',
#             epsilon_is(trade_baseline,e),
#             1/alpha_is(trade_baseline)).ravel()
# ax.scatter(data['epsilon'],data['term_3'])
# ax.set_ylabel('alpha_cf/alpha_baseline')
# ax.set_xlabel('epsilon over alpha')

# data = data.loc['USA']
ax.tick_params(axis='both', which='major', labelsize=20)
ax.scatter(b.co2_intensity.value,data['term_3']*100-100)
# ax.set_xlabel('CO2 intensity')

ax.set_xscale('log')
# ax.set_yscale('log')

# plt.savefig(save_path+'growth_rate_em_intensity_term_3.png',format='png')
# 
plt.show()

#%% Alpha of epsilon term 3
...
#             epsilon_is(trade_baseline,e)).ravel()
# ax.scatter(data['epsilon'],data['term_3'])
# ax.set_xlabel(r'$\epsilon_{is}$')
# ax.set_ylabel('Term 3')
data['alpha'] = np.einsum('is->is',
           alpha_is(trade_baseline)).ravel()
data['epsilon'] = np.einsum('is->is',
           epsilon_is(trade_baseline,e)).ravel()
ax.scatter(data['epsilon'],data['alpha'])
ax.set_ylabel('alpha')
ax.set_xlabel('epsilon')

# data = data.loc['USA']

# ax.scatter(b.co2_intensity.value,data['term_3'])
# ax.set_xlabel('CO2 intensity')
...
# ax.set_xlabel(r'$\epsilon_{is}$')
# ax.set_ylabel('Term 3')
data['carbon_intensity'] = b.co2_intensity.value.ravel()
data['epsilon'] = np.einsum('is->is',
           epsilon_is(trade_baseline,e)).ravel()
ax.scatter(data['carbon_intensity'],data['epsilon'])
...
# ax.scatter(b.co2_intensity.value,data['term_3'])
ax.scatter(b.output.values,data['term_3'])
# ax.set_xlabel('CO2 intensity')
